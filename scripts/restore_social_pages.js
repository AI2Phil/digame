...
// Social Networking pages to restore from archived directory
const socialPages = [
  'analytics.js',
  'collaboration.js', 
  'events.js',
  'index.js',
  'learning-partners.js',
  'mentorship.js',
  'network.js',
  'peer-matching.js'
  // Note: forums.js already exists as forums.tsx, so we skip it
];

const archivedDir = 'frontend/pages_archived_20250717_193641/social';
const targetDir = 'frontend/src/pages/social';
...
socialPages.forEach(filename => {
  try {
    ...
    // Check if target already exists (avoid overwriting forums.tsx)
    if (fs.existsSync(targetPath)) {
      console.log(`⚠️  Target file already exists, skipping: ${targetPath}`);
      return;
    }
...
console.log(`\n📊 Social Networking Restoration Summary:`);
console.log(`✅ Successfully restored: ${successCount} pages`);
console.log(`❌ Errors: ${errorCount} pages`);
console.log(`📁 Target directory: ${targetDir}`);
console.log(`📝 Note: forums.tsx already exists and was preserved`);

if (successCount > 0) {
  console.log(`\n🎉 Social Networking section restoration complete!`);
  console.log(`All ${successCount} new pages are now available at /social/* routes`);
}