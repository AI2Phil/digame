...
async function startTestServer() {
  try {
    ...
    // Copy test config
    const testConfigPath = path.join(__dirname, '..', 'frontend', 'next.config.test.js');
    ...
    fs.writeFileSync(path.join(__dirname, '..', 'frontend', '.test-port'), port.toString());
    ...
    process.on('SIGTERM', () => {
      ...
      // Clean up port file
      try {
        fs.unlinkSync(path.join(__dirname, '..', 'frontend', '.test-port'));
      } catch (e) {
