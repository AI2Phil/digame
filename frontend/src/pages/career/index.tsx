...
const CareerDevelopment: React.FC = () => {
  ...
  const getMockOpportunities = () => ({
    ...
    marketInsights: {
      ...
      in_demand_skills: [{ skill: 'AI/Machine Learning', growth: '+45%', avg_salary: '$125,000' }],
    },
  });
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Accelerate your career growth with personalized insights"
        />
                          ...
                          <div key={skillIndex} className="border border-gray-200 rounded-lg p-4">
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <div className="flex items-center space-x-2">
                                ...
                                <span
                                  className={`px-2 py-1 text-xs font-medium rounded-full ${getDemandColor(skill.demand)}`}
                                >
                                  {skill.demand}
                                </span>
                              </div>
                        ...
                        <div className="space-y-2">
                          ...
                          {goal.milestones.map((milestone, index) => (
                            <div key={index} className="flex items-center space-x-3">
                              <CheckCircle
                                className={`w-4 h-4 ${milestone.completed ? 'text-green-600' : 'text-gray-400'}`}
                              />
                              <span
                                className={`text-sm ${milestone.completed ? 'text-gray-900 line-through' : 'text-gray-700'}`}
                              >
                                {milestone.task}
                              </span>
                            </div>
                        ...
                        <div className="flex items-center space-x-2 mb-4">
                          <div className="flex items-center">
                            {[...Array(5)].map((_, i) => (
                              <Star
                                key={i}
                                className={`w-4 h-4 ${i < Math.floor(path.rating) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
                              />
                            ))}
                          </div>
