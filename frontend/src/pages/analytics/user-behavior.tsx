...
interface UserBehaviorAnalyticsPageProps {
  isDemoMode?: boolean;
  onLogout?: () => void;
}

const UserBehaviorAnalyticsPage: React.FC<UserBehaviorAnalyticsPageProps> = ({ 
  isDemoMode = false, 
  ...
  useEffect(() => {
    const fetchBehaviorData = async () => {
      try {
        ...
        // Try multiple backend endpoints for comprehensive behavior data
        const [userBehavior, performanceMetrics, advancedAnalytics] = await Promise.allSettled([
          fetch(`http://localhost:8000/advanced-analytics/user-behavior?days=${days}`, {
        ...
        if (userBehavior.status === 'fulfilled' && userBehavior.value.ok) {
          const behaviorResult = await userBehavior.value.json();
          if (behaviorResult.success && behaviorResult.data) {
            data.patterns = behaviorResult.data.patterns || [
              {
                pattern: 'Deep Work Sessions',
                description: 'You tend to have longer focused work periods in the morning',
                impact: 'High' as const,
                confidence: 92,
                recommendation: 'Schedule your most important tasks between 9-11 AM for maximum productivity'
              },
              {
                pattern: 'Collaboration Preference',
                description: 'You work more effectively in small group settings',
                impact: 'Medium' as const,
                confidence: 78,
                recommendation: 'Limit meeting sizes to 3-4 people for better engagement'
              },
              {
                pattern: 'Break Timing',
                description: 'You take breaks every 90 minutes on average',
                impact: 'Low' as const,
                confidence: 85,
                recommendation: 'Continue with your natural break rhythm to maintain focus'
              }
            ];
            
            if (behaviorResult.data.work_patterns) {
              data.workPatterns = {
                peakHours: behaviorResult.data.work_patterns.peak_hours || ['9:00-11:00', '14:00-16:00'],
                preferredBreaks: behaviorResult.data.work_patterns.break_duration || 15,
                focusBlocks: behaviorResult.data.work_patterns.focus_duration || 90,
                multitaskingTendency: behaviorResult.data.work_patterns.multitasking || 'Low'
              };
            }
          }
        }

        // If no real data, use comprehensive fallback data
        if (data.patterns.length === 0) {
          data.patterns = [
            {
              pattern: 'Morning Productivity Peak',
              description: 'Your productivity is highest between 9-11 AM with 95% efficiency',
              impact: 'High',
              confidence: 92,
              recommendation: 'Schedule your most challenging tasks during morning hours for optimal results'
            },
            {
              pattern: 'Focused Work Blocks',
              description: 'You maintain deep focus for 90-minute intervals before needing breaks',
              impact: 'High',
              confidence: 88,
              recommendation: 'Structure your day around 90-minute focused work sessions with 15-minute breaks'
            },
            {
              pattern: 'Collaboration Style',
              description: 'You perform best in small team settings (2-4 people) rather than large meetings',
              impact: 'Medium',
              confidence: 78,
              recommendation: 'Advocate for smaller, more focused meetings to maximize your contribution'
            },
            {
              pattern: 'Afternoon Energy Dip',
              description: 'Your energy and focus decrease significantly after 3 PM',
              impact: 'Medium',
              confidence: 85,
              recommendation: 'Use afternoon hours for lighter tasks like email, planning, or administrative work'
            },
            {
              pattern: 'Task Switching Sensitivity',
              description: 'Frequent task switching reduces your productivity by up to 25%',
              impact: 'High',
              confidence: 90,
              recommendation: 'Batch similar tasks together and minimize context switching throughout the day'
            },
            {
              pattern: 'Creative Problem Solving',
              description: 'Your best creative insights occur during low-pressure, unstructured time',
              impact: 'Medium',
              confidence: 72,
              recommendation: 'Schedule regular "thinking time" without specific deliverables to foster innovation'
            }
          ];
        }
        ...
        // Use comprehensive fallback data on error
        setBehaviorData({
          patterns: [
            {
              pattern: 'Morning Productivity Peak',
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-xl font-bold text-gray-900">User Behavior Analytics</h1>
              {isDemoMode && (
          ...
          <div className="mb-6 bg-amber-50 border border-amber-200 rounded-lg p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h4 className="text-sm font-medium text-amber-800">Backend Connection Issue</h4>
                <p className="text-sm text-amber-700">
                  {error} - Displaying fallback data for demonstration purposes.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="bg-white rounded-lg p-4">
              ...
              <p className="text-sm text-gray-600">
                Schedule important tasks during your peak hours (9-11 AM) for maximum productivity.
              </p>
            </div>
            <div className="bg-white rounded-lg p-4">
              <div className="text-2xl mb-2">🎯</div>
              ...
              <p className="text-sm text-gray-600">
                Your ideal focus block is 90 minutes. Use this for deep work sessions.
              </p>
            </div>
            <div className="bg-white rounded-lg p-4">
              <div className="text-2xl mb-2">🤝</div>
              ...
              <p className="text-sm text-gray-600">
                You work best in small groups. Consider limiting meeting sizes for better
                engagement.
              </p>
            </div>
          </div>
