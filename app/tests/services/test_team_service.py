...
from app.models.team import Team, TeamMember, TeamRoleEnum # Import necessary models
from app.schemas import team_schemas as schemas
from app.services.team_service import TeamService
from app.crud import user_crud, team_crud # For direct CRUD ops in setup if needed
...
def regular_user(db: SessionType) -> User:
    ...
    setattr(user, 'username', "regularuser")  # type: ignore
    setattr(user, 'email', "regular@example.com")  # type: ignore
...
def team_creator_user(db: SessionType) -> User:
    user = User()  # type: ignore
    setattr(user, 'username', "creatoruser")  # type: ignore
    setattr(user, 'email', "creator@example.com")  # type: ignore
    setattr(user, 'hashed_password', "password")  # type: ignore
    db.add(user)
    db.commit()
    db.refresh(user)
...
def another_user(db: SessionType) -> User:
    user = User()  # type: ignore
    setattr(user, 'username', "anotheruser")  # type: ignore
    setattr(user, 'email', "another@example.com")  # type: ignore
    setattr(user, 'hashed_password', "password")  # type: ignore
    db.add(user)
    db.commit()
    db.refresh(user)
...
def sample_team(db: SessionType, team_service: TeamService, team_creator_user: User) -> Team:
    ...
    team = team_service.create_team(team_create=team_schema, current_user_id=getattr(team_creator_user, 'id'))  # type: ignore
...
def test_service_create_team(team_service: TeamService, team_creator_user: User):
    ...
    db_team = team_crud.get_team(team_service.db, getattr(team, 'id'))  # type: ignore
    assert db_team is not None
    assert len(getattr(db_team, 'members', [])) == 1  # type: ignore
...
def test_service_create_team_with_initial_members(team_service: TeamService, team_creator_user: User, regular_user: User):
    ...
    db_team = team_crud.get_team(team_service.db, getattr(team, 'id'))  # type: ignore
    assert len(getattr(db_team, 'members', [])) == 2  # type: ignore
    assert any(getattr(m, 'user_id') == getattr(team_creator_user, 'id') and getattr(m, 'role') == TeamRoleEnum.ADMIN for m in getattr(db_team, 'members', []))  # type: ignore
    assert any(getattr(m, 'user_id') == getattr(regular_user, 'id') and getattr(m, 'role') == TeamRoleEnum.MEMBER for m in getattr(db_team, 'members', []))  # type: ignore

# Test Team Retrieval
def test_service_get_team(team_service: TeamService, sample_team: Team):
    retrieved_team = team_service.get_team(team_id=getattr(sample_team, 'id'))  # type: ignore
...
def test_service_update_team_by_creator(team_service: TeamService, sample_team: Team, team_creator_user: User):
    ...
    updated_team = team_service.update_team_details(
        team_id=getattr(sample_team, 'id'),  # type: ignore
        team_update=update_schema,
        current_user_id=getattr(team_creator_user, 'id')  # type: ignore
    )
...
def test_service_update_team_by_team_admin(db: SessionType, team_service: TeamService, sample_team: Team, another_user: User, team_creator_user: User):
    ...
    team_crud.create_team_member(db, team_id=getattr(sample_team, 'id'), member=schemas.TeamMemberCreate(user_id=getattr(another_user, 'id'), role=schemas.TeamRoleEnumSchema.ADMIN))  # type: ignore

    update_schema = schemas.TeamUpdate(description="Updated by team admin")
    updated_team = team_service.update_team_details(
        team_id=getattr(sample_team, 'id'),  # type: ignore
        team_update=update_schema,
        current_user_id=getattr(another_user, 'id')  # type: ignore
    )
...
def test_service_update_team_unauthorized(team_service: TeamService, sample_team: Team, regular_user: User):
    ...
    with pytest.raises(HTTPException) as exc_info:
        team_service.update_team_details(
            team_id=getattr(sample_team, 'id'),  # type: ignore
            team_update=update_schema,
            current_user_id=getattr(regular_user, 'id')  # type: ignore
        )
...
def test_service_delete_team_by_creator(team_service: TeamService, sample_team: Team, team_creator_user: User):
    response = team_service.remove_team(team_id=getattr(sample_team, 'id'), current_user_id=getattr(team_creator_user, 'id'))  # type: ignore
    ...
    with pytest.raises(HTTPException): # Expect 404 when trying to get deleted team
        team_service.get_team(team_id=getattr(sample_team, 'id'))  # type: ignore
...
def test_service_add_team_member_by_admin(team_service: TeamService, sample_team: Team, team_creator_user: User, regular_user: User):
    ...
    db_team = team_crud.get_team(team_service.db, getattr(sample_team, 'id'))  # type: ignore
    assert len(getattr(db_team, 'members', [])) == 2  # type: ignore
...
def test_service_list_team_members(team_service: TeamService, sample_team: Team):
    members = team_service.list_team_members(team_id=getattr(sample_team, 'id'))  # type: ignore
...
def test_service_update_member_role_by_admin(db: SessionType, team_service: TeamService, sample_team: Team, team_creator_user: User, regular_user: User):
    ...
    team_crud.create_team_member(db, team_id=getattr(sample_team, 'id'), member=schemas.TeamMemberCreate(user_id=getattr(regular_user, 'id'), role=schemas.TeamRoleEnumSchema.MEMBER))  # type: ignore
    ...
    updated_member = team_service.update_team_member_role(
        team_id=getattr(sample_team, 'id'),  # type: ignore
        user_id_to_update=getattr(regular_user, 'id'),  # type: ignore
...
def test_service_remove_member_by_admin(team_service: TeamService, sample_team: Team, team_creator_user: User, regular_user: User):
    # Add regular_user as a member first
    team_crud.create_team_member(team_service.db, team_id=getattr(sample_team, 'id'), member=schemas.TeamMemberCreate(user_id=getattr(regular_user, 'id'), role=schemas.TeamRoleEnumSchema.MEMBER))  # type: ignore
    ...
    response = team_service.remove_member_from_team(
        team_id=getattr(sample_team, 'id'),  # type: ignore
        user_id_to_remove=getattr(regular_user, 'id'),  # type: ignore
        current_user_id=getattr(team_creator_user, 'id')  # type: ignore
    )
    assert response["message"] == "Team member removed successfully"

    db_team_after_removal = team_crud.get_team(team_service.db, getattr(sample_team, 'id'))  # type: ignore
    assert len(getattr(db_team_after_removal, 'members', [])) == 1  # type: ignore
...
def test_service_get_team_performance_analytics(team_service: TeamService, sample_team: Team, team_creator_user: User):
    ...
    analytics = team_service.get_team_performance_analytics(team_id=getattr(sample_team, 'id'), current_user_id=getattr(team_creator_user, 'id'))  # type: ignore
...
def test_service_get_team_analytics_unauthorized_non_member(team_service: TeamService, sample_team: Team, another_user: User):
    ...
    with pytest.raises(HTTPException) as exc_info:
        team_service.get_team_performance_analytics(team_id=getattr(sample_team, 'id'), current_user_id=getattr(another_user, 'id'))  # type: ignore
    assert exc_info.value.status_code == 403
...
def test_service_add_team_performance_metric_authorized(team_service: TeamService, sample_team: Team, team_creator_user: User):
    ...
    metric = team_service.add_team_performance_metric(
        team_id=getattr(sample_team, 'id'), metric_create=metric_create, current_user_id=getattr(team_creator_user, 'id')  # type: ignore
    )
...
def test_service_add_team_metric_unauthorized(team_service: TeamService, sample_team: Team, regular_user: User):
    ...
    with pytest.raises(HTTPException) as exc_info:
        team_service.add_team_performance_metric(
            team_id=getattr(sample_team, 'id'), metric_create=metric_create, current_user_id=getattr(regular_user, 'id')  # type: ignore
        )
    assert exc_info.value.status_code == 403
