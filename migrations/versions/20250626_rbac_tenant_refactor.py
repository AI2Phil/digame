        ...
        try:
            ...
            try:
                ...
                if "already exists" not in str(e).lower():
                    print(f"Warning: Could not create index ix_users_tenant_id: {e}")
            try:
                op.create_foreign_key('fk_users_tenant_id', 'users', 'tenants', ['tenant_id'], ['id'])
            except Exception as e:
                if "already exists" not in str(e).lower():
                    print(f"Warning: Could not create foreign key fk_users_tenant_id: {e}")
        except Exception as e:
            if "already exists" in str(e).lower():
                print("tenant_id already exists in users table, skipping")
            else:
                print(f"Warning: Could not add tenant_id to users table: {e}")
    else:
        print("tenant_id already exists in users table, skipping")
    ...
    if 'tenant_id' not in existing_role_columns:
        ...
        try:
            ...
            try:
                ...
                if "already exists" not in str(e).lower():
                    print(f"Warning: Could not create index ix_roles_tenant_id: {e}")
            try:
                op.create_foreign_key('fk_roles_tenant_id', 'roles', 'tenants', ['tenant_id'], ['id'])
            except Exception as e:
                if "already exists" not in str(e).lower():
                    print(f"Warning: Could not create foreign key fk_roles_tenant_id: {e}")
        except Exception as e:
            if "already exists" in str(e).lower():
                print("tenant_id already exists in roles table, skipping")
            else:
                print(f"Warning: Could not add tenant_id to roles table: {e}")
    else:
        print("tenant_id already exists in roles table, skipping")
    ...
    if 'user_roles_enhanced' not in existing_tables:
        ...
        try:
            ...
            try:
                ...
                if "already exists" not in str(e).lower():
                    print(f"Warning: Could not create index ix_user_roles_enhanced_id: {e}")
            
            try:
                op.create_index(op.f('ix_user_roles_enhanced_role_id'), 'user_roles_enhanced', ['role_id'], unique=False)
            except Exception as e:
                if "already exists" not in str(e).lower():
                    print(f"Warning: Could not create index ix_user_roles_enhanced_role_id: {e}")
            
            try:
                op.create_index(op.f('ix_user_roles_enhanced_tenant_id'), 'user_roles_enhanced', ['tenant_id'], unique=False)
            except Exception as e:
                if "already exists" not in str(e).lower():
                    print(f"Warning: Could not create index ix_user_roles_enhanced_tenant_id: {e}")
            
            try:
                op.create_index(op.f('ix_user_roles_enhanced_user_id'), 'user_roles_enhanced', ['user_id'], unique=False)
            except Exception as e:
                if "already exists" not in str(e).lower():
                    print(f"Warning: Could not create index ix_user_roles_enhanced_user_id: {e}")
            ...
            if "already exists" in str(e).lower():
                ...
                print(f"Warning: Could not create user_roles_enhanced table: {e}")
    else:
        print("user_roles_enhanced table already exists, skipping")
